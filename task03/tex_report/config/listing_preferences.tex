% define PowerShell as language (credit: https://github.com/rmainer/latex-listings-powershell)
...
% syntax highlighting colors from eclipse IDE
\definecolor{EclipseIdentifier}{HTML}{000080}
\definecolor{EclipseKeyword}{HTML}{0000FF}
\definecolor{EclipseComment}{HTML}{808080}
\definecolor{EclipseString}{HTML}{4D4D4D}
\definecolor{CodeBackground}{HTML}{F9F9F9}
\definecolor{EclipseNumVar}{HTML}{FF8000}

% listings package reference: https://en.wikibooks.org/wiki/LaTeX/Source_Code_Listings
% full parameter reference: https://texdoc.org/serve/listings.pdf/0

% define style: Modified parameters for this project, Java code
\lstdefinestyle{customJava}
{
	language=Java,					
	basicstyle=\small\ttfamily, 		
	keywordstyle=\color{EclipseKeyword}\bfseries,		
	identifierstyle=\color{EclipseIdentifier}\ttfamily,
	commentstyle=\color{EclipseComment}, 
	stringstyle=\color{EclipseString}\ttfamily,
...
% define style: Modified parameters for this project, Python code
...
% define style: Modified parameters for this project, PowerShell code
\lstdefinestyle{customPS}
{
	language=PowerShell,					
	basicstyle=\small\ttfamily, 		
	keywordstyle=\color{EclipseKeyword}\bfseries,		
...
% define style: Modified parameters for this project, Plain output (console prints)
\lstdefinestyle{customPlain}
{
	language=Python, % still has "Python" argument, because there's no "unstyled" language param	
	basicstyle=\footnotesize\ttfamily, 			
...
\lstdefinestyle{customJSON}{
    ...
    basicstyle=\small\ttfamily, 		
	keywordstyle=\color{EclipseKeyword}\bfseries,		
	identifierstyle=\color{EclipseComment}\ttfamily,
	commentstyle=\color{EclipseComment}, 
	stringstyle=\color{EclipseString}\ttfamily,
    backgroundcolor=\color{CodeBackground},
    rulecolor=\color{black},
	...
	xleftmargin=8mm,
    literate=
     *{0}{{{\color{EclipseNumVar}0}}}{1}
      {1}{{{\color{EclipseNumVar}1}}}{1}
      {2}{{{\color{EclipseNumVar}2}}}{1}
      {3}{{{\color{EclipseNumVar}3}}}{1}
      {4}{{{\color{EclipseNumVar}4}}}{1}
      {5}{{{\color{EclipseNumVar}5}}}{1}
      {6}{{{\color{EclipseNumVar}6}}}{1}
      {7}{{{\color{EclipseNumVar}7}}}{1}
      {8}{{{\color{EclipseNumVar}8}}}{1}
      {9}{{{\color{EclipseNumVar}9}}}{1}
      {:}{{{\color{EclipseComment}{:}}}}{1}
      {,}{{{\color{EclipseComment}{,}}}}{1}
      {\{}{{{\color{black}{\{}}}}{1}
      {\}}{{{\color{black}{\}}}}}{1}
      {[}{{{\color{black}{[}}}}{1}
...
% Note: These macros seem to have issues handling commas within any part of it, probably due to the nature of argument passing. Keep this in mind, as it is going to cause issues. 

% set default style, being Java.
\lstset{style=customJava}

% macro for fast code inclusion from file.
% will produce the listing and directory to this listing.

% usage: modify original code file from repo and place it into latex "code"-dir
% remove unneccessary info and 

% 1 - {"Java" or "Python"}
% 2 - {Path to code file from latex root}
% 3 - {Path to original code file from repo root} -> Cannot contain LaTeX math symbols!
% 4 - {caption name}
% 5 - {label name}
\newcommand{\codefromfile}[5]{\lstinputlisting[caption=#4, label=#5, style=custom#1]{#2} \hspace*{\fill} \FolderOpen \enspace \directory{#3}}

% 1 - {"Java" or "Python"}
% 2 - {Path to code file from latex root}
% 3 - {caption name}
% 4 - {label name}
\newcommand{\codefromfilenodir}[4]{\lstinputlisting[caption=#3, label=#4, style=custom#1]{#2}}

% macro for console print inclusion from file.

% 1 - {Path to code file from latex root}
% 2 - {caption name}
% 3 - {label name}
\newcommand{\printfromfile}[3]{\lstinputlisting[caption=#2, label=#3, style=customPlain]{#1}}

% 1 - {Path to code file from latex root}
% 2 - {Path to original code file from repo root} -> Cannot contain LaTeX math symbols!
% 3 - {caption name}
% 4 - {label name}
\newcommand{\printfromfiledir}[4]{\lstinputlisting[caption=#3, label=#4, style=customPlain]{#1} \hspace*{\fill} \FolderOpen \enspace \directory{#2}}
